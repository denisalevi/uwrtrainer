...
/** Trainer management of the active team's weekly practice slots (add, edit, pause). */
export async function PracticeSlotsSettings({ teamId }: { teamId: string | null }) {
    ...
    <div className="space-y-4">
      {slots.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-100">
          {slots.map((s) => (
            <li key={s.id} className={cn("flex flex-col gap-3 py-3 first:pt-0", !s.active && "opacity-50")}>
              ...
              <details className="group/edit">
                ...
                <form action={updateSlot} className="mt-3 space-y-3">
                  ...
                  <div>
                    ...
                    <Input id={`label-${s.id}`} name="label" defaultValue={s.label} required />
                  </div>
      ...
      <details className="group/add border-t border-slate-100 pt-3">
        <summary className="cursor-pointer text-sm font-medium text-teal-700">
          + {t("slots.add")}
        </summary>
        <form action={addSlot} className="mt-3 space-y-3">
          <div>
