...
- [x] `runWeeklyReconcileIfDue` (src/lib/missed.ts): instead of only reconciling the single
...
- [x] Keep read-then-write guard semantics (same race window as today; note it in a comment —
      multi-instance locking is out of scope).
- [x] Test (src/lib/missed.test.ts, in-memory fake): a two-week downtime gap reconciles both
      closed weeks, each with its own frozen targets.

## 2. Duplicate-attendance race backstop
- [x] Check for legitimate same-(user, slot, date) multi-DONE rows — audit updateSession /
      logSession / strength paths.
- [x] Additive migration `prisma/migrations/<ts>_attendance_dedup_index/migration.sql`:
      ...
      "status"='DONE';`
- [x] `logPracticeAttendance` (src/app/actions/training.ts): tolerate the unique violation
      (catch P2002 and fall through — the row already exists, which is the desired end state).

## Verification
- [x] `npx vitest run` and `npm run build` after each item.
