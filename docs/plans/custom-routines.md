...
- **Per-exercise measurement type**: a dropdown per routine exercise, defaulting to
  **kg × reps**. Initial option set:
  ...
  - `SECONDS` — timed set (plank, holds — needed for the future bodyweight workouts, e.g. 3×30 s)
  - `KG_SECONDS` — weighted hold (farmer carry, weighted plank) — cheap to include since the two
    axes already exist
- Future bodyweight workouts will be built as (team-)routines using `REPS` + `SECONDS` — no
  separate engine.
...
model Routine {
  ...
  name      String
  exercises String               // JSON: [{ name, measure, sets: [{ reps?, weight?, seconds? }], restSeconds? }]
...
- `LoggerDay` grows a per-suggestion `measure`; `SetVal` gets an optional `seconds` field.
  The set row renders inputs per measure (kg+reps / reps / seconds / kg+seconds). The details
  JSON stores `seconds` next to weight/reps; `StrengthWorkoutView` formats accordingly
  (`3 × 30 s`, `24 kg × 40 s`).
