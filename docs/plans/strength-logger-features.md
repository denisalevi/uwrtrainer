...
Resume notes for whoever continues.

## 1. Session timer + duration prefill (owner request)

- [ ] `strength-workout-logger.tsx`: record wall-clock `startedAt` (ms epoch) in the
...
- [ ] Elapsed readout near the duration field: recompute `Date.now() - startedAt` on a 1s
      tick **and** on `visibilitychange` (phones lock between sets) — never an interval counter.
- [ ] Finish: if `durationMin` empty and `startedAt` present, prefill `round(elapsed min)`.
...
- [ ] In `setField`, when a reps value is entered and afterwards every set of the line has
      reps: auto-set `done`. Only auto-set, never auto-clear (deleting a rep doesn't untick).
- [ ] Manual toggles record the line key in a `doneTouched` ref (component state only);
      auto-set skips touched lines, so a manual untick sticks.

## 3. Configurable weight rounding (#4)

- [ ] Schema: `User.weightRounding` String default `"DOWN"` (`EXACT|DOWN|NEAREST|UP`),
...
- [ ] `src/lib/strength.ts`: pure `roundWeight(value, mode, increment)`; thread a
...
- [ ] Settings section (pattern: rest-timer section) + server action (DAL re-check).
- [ ] Thread user pref into `/strength` page + `/strength/log` page set computation.
- [ ] `EXACT`: unrounded weights + i18n info note (round down at the rack; below ~20 kg use
...
- Fallback if time runs out: ship DOWN/NEAREST/UP + increment, leave EXACT out. — shipped in full incl. EXACT? see checkboxes.
